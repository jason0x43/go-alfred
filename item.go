...
func (b byFuzzyScore) Less(i, j int) bool {
	if !b[i].fuzzyScored {
		b[i].fuzzyScore = fuzzyScore(b[i].Title, b[i].fuzzyTest)
		b[i].fuzzyScored = true
	}
	if !b[j].fuzzyScored {
		b[j].fuzzyScore = fuzzyScore(b[j].Title, b[j].fuzzyTest)
		b[j].fuzzyScored = true
	}
